...
def export_diagram(circuit: stim.Circuit, initial_basis: str):
    ...
    with open(
        f"simulation/z_ZX_interleaved/figs/circuit_detslice-with-ops_{initial_basis}.svg",
        "w",
        encoding="utf-8",
    ) as f:
        f.write(str(diagram))
...
def export_diagram_full(circuit: stim.Circuit, initial_basis: str):
    ...
    with open(
        f"simulation/z_ZX_interleaved/figs/circuit_detslice-with-ops_full_{initial_basis}.svg",
        "w",
        encoding="utf-8",
    ) as f:
        f.write(str(diagram))
    ...
    diagram1 = circuit.without_noise().diagram(
        "detslice-with-ops-svg", tick=range(39, 56), rows=6
    )
    with open(
        "simulation/z_ZX_interleaved/figs/circuit_detslice-with-ops1.svg",
        "w",
        encoding="utf-8",
    ) as f:
    ...
    # SVGからPDFに変換
    cairosvg.svg2pdf(
        url="simulation/z_ZX_interleaved/figs/circuit_detslice-with-ops1.svg",
        write_to="simulation/z_ZX_interleaved/figs/circuit_detslice-with-ops1.pdf",
    )
