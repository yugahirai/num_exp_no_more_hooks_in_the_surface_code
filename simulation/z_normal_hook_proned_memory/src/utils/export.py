...
def export_diagram(circuit: stim.Circuit, initial_basis: str):
    ...
    with open(
        f"simulation/z_normal_hook_proned_memory/figs/circuit_detslice-with-ops_{initial_basis}.svg",
        "w",
        encoding="utf-8",
    ) as f:
        f.write(str(diagram))
...
def export_diagram_full(circuit: stim.Circuit, initial_basis: str):
    ...
    with open(
        f"simulation/z_normal_hook_proned_memory/figs/circuit_detslice-with-ops_full_{initial_basis}.svg",
        "w",
        encoding="utf-8",
    ) as f:
        f.write(str(diagram))
