...
def main(d: int, p: float, l: int, z_detector: bool = True, x_detector: bool = True):
    ...
    for round_z_init in range(MAX_ROUND_Z_INIT):
        ...
        if round_z_init == 0:
            ...
            z_intialization.append_body_detector()

    # XX_measurement
    xx_measurement = XX_measurement(
        circuit, alloc.loc, d, p, 0, record, l, z_detector, x_detector
    )
    ...
    for round_xx_measurement in range(MAX_ROUND_XX_MEASUREMENT):
        xx_measurement.append_syndrome(
            round_xx_measurement, round_xx_measurement, MAX_ROUND_XX_MEASUREMENT
        )
        if round_xx_measurement == 0:
            xx_measurement.append_head_detector()
        else:
            xx_measurement.append_body_detector()
    # shrink
    shrk = shrink(
        circuit,
        alloc.loc,
    ...
    for round_shrink in range(MAX_ROUND_SHRINK):
        ...
        if round_shrink == 0:
            ...
            for qubit_index in shrk.previous_data_list:
