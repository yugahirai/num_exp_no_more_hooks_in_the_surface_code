...
class Z_measurement(constbase.ConstBase):
    def __init__(
        self,
        ...
        if self.z_detector:
            for qubit_index in self.z_ancilla_list:
                ...
                if y == self.Q1_MAX_Y - 1:
                    ...
                    rec_prev_lower = self.record.get_record(
                        (x, y + 2, self.record.get_t() - 2)
                    )
                    if all(
                        r is not None for r in [found_rec, rec_prev, rec_prev_lower]
                    ):
                        self.append_detector(found_rec + [rec_prev, rec_prev_lower])
                elif y == self.Q1_MIN_Y - 1:
                    found_rec = self.search_around_detector(qubit_index)
                    rec_prev_lower = self.record.get_record(
                        (x, y + 2, self.record.get_t() - 2)
                    )
                    if all(r is not None for r in [found_rec, rec_prev_lower]):
