...
def main(d: int, p: float, l: int, z_detector: bool = True, x_detector: bool = True):
    ...
    for round_z_init in range(MAX_ROUND_Z_INIT):
        ...
        if round_z_init % 2 == 1:
            for qubit_index in z_intialization.z_ancilla_list:
                pos = z_intialization.loc.get_location(qubit_index)
                x = pos[0]
                y = pos[1]
                if x == z_intialization.Q1_MAX_X + 1:
            ...
            for qubit_index in z_intialization.x_ancilla_list:
                pos = z_intialization.loc.get_location(qubit_index)
                x = pos[0]
                y = pos[1]
                if (
                    x == z_intialization.Q1_MAX_X + 1
                    and y == z_intialization.Q1_MIN_Y - 1
                ):
                    target_rec = record.get_record((x, y, record.get_t() - 1))
                    if target_rec is not None:
                        circuit.append(
